...
class BiConvGRU(nn.Module):
    def __init__(self, input_channels) :
        ...
        self.bidirection_conv = nn.Conv2d(input_channels, input_channels, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        sequence = []
        for i in range(x.shape[1]):
            image = torch.tanh(self.bidirection_conv(sequence_forward[i]))
            sequence.append(image)

        last_sequence = sequence[-1] # 'b,8,c,h,w'
