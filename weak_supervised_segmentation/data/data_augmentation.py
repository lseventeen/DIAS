...
class RandomRotate:
    ...
    def __call__(self, m):
        ...
        if m.ndim == 3:
            m = rotate(m, angle, axes=(1, 2), reshape=False, order=self.order, mode=self.mode, cval=-1)
        else:
            m = rotate(m, angle, axes=(0, 1), reshape=False, order=self.order, mode=self.mode, cval=-1)
...
class ElasticDeformation:
    ...
    def __call__(self, m):
        if self.random_state.uniform() < self.execution_probability:
            ...
            dy, dx = [
                gaussian_filter(
                    self.random_state.randn(1,volume_shape[1],volume_shape[2]),
                    # a,
